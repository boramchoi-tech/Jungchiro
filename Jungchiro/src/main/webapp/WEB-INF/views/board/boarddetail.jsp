...
<script type="text/javascript">
	...
	function updateReply(reply_seq) {
		var board_seq = $("#updateForm" + reply_seq).find("input[name='board_seq']").val();
		var reply_seq = $("#updateForm" + reply_seq).find("input[name='reply_seq']").val();
		var reply_content = $("#updateForm" + reply_seq).find("textarea[name='reply_content']").val();
...
<body>
	...
	<form name="readForm" role="form" method="post">
		...
		<input type="hidden" id="perPageNum" name="perPageNum" value="${search.perPageNum}">
		...
		<c:if test="${principal.member_seq eq board.member_seq }">
			<tr>
				<td colspan="8" align="right"><input type="button" value="수정" onclick="location.href='/poli/boardupdateform.do?board_seq=${board.board_seq}&member_seq=${board.member_seq }'">
						...
						<input type="button" value="글 목록" onclick="location.href ='/poli/boardlist.do?page=${search.page}&perPageNum=${search.perPageNum}&searchType=${search.searchType}&keyword=${search.keyword}'">
				</td>
			</tr>
		</c:if>
	</table>
	...
	function insertFav(){
		...
		var member_seq = document.getElementById("member_seq").value;
		var board_seq = document.getElementById("board_seq").value;
		console.log("member_seq :" + member_seq + "board_seq : " + board_seq);
